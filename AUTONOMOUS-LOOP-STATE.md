...
**Current pass:** 2026-06-07 Cycle 159 background work action hints
**Last commit before pass:** `568ea82` (`feat(diagnostics): include live background receipts`)
...
## Cycle 143 Result - 2026-06-07

- Added `localStorageSurfaces` coverage to
  `docs/privacy/data-safety.json`.
- Added a Local Storage Ledger to `docs/privacy/data-safety.md`.
- Extended `tools/privacy_data_safety_check.py` so every local storage row must
  include data types, collection/sharing status, user control, retention,
  deletion path, backup/transfer posture, and existing source-file paths.
- Covered DataStore, Room, community identity/vote SharedPreferences, widget
  and selection preferences, live wallpaper prefs/media, crash diagnostics,
  Coil/audio caches, offline favorite files, generated wallpaper files,
  community recording temp files, share-out temp files, and Aura Originals
  files.
- Extended focused tests for missing local storage source paths, missing docs
  rows, and unsupported backup statuses.
- Updated release dry-run docs, supply-chain docs, research, roadmap,
  changelog, completion, and loop-state docs.
...
## Cycle 147 Result - 2026-06-07

- Added `docs/distribution/alt-store-metadata.md` as the owner-facing
  alternative-store disclosure packet for GitHub, Obtainium, IzzyOnDroid, and
  F-Droid mainline.
- Added `docs/distribution/alt-store-metadata.json` as the checked contract
  for channel status, anti-feature notes, proprietary dependency markers,
  manifest permission disclosures, reviewed network-service rows, Izzy
  submission notes, and source URLs.
- Added `tools/alt_store_metadata_check.py` plus focused backend tests for the
  live packet, missing permission rows, missing network-service rows, missing
  proprietary dependency markers, and stale F-Droid readiness claims.
- Wired the gate into verify and release workflows before Android build work.
- Updated release dry-run, release signing, supply-chain, channel strategy,
  README, research, roadmap, changelog, completion, and loop-state docs.
- Cycle 147 verification: alternative-store metadata check, F-Droid blocked
...
## Cycle 149 Result - 2026-06-07

- Added `docs/distribution/sbom-readiness.md` as the owner-facing SBOM
  readiness packet.
- Added `docs/distribution/sbom-readiness.json` as the checked contract for the
  deferred-until-N-1 SBOM decision, current release evidence paths, current
  release artifact names, future CycloneDX/SPDX artifact names, future scope,
  workflow commands, and source URLs.
- Added `tools/sbom_readiness_check.py` plus focused backend tests for the live
  packet, status drift, missing future artifact documentation, missing workflow
  command, and missing evidence path.
- Wired the gate into verify and release workflows before Android build work.
- Updated release dry-run, release signing, supply-chain, release metadata,
  ...
  and loop-state docs.
- Cycle 149 verification: SBOM readiness check, release metadata consistency
...
## Cycle 152 Result - 2026-06-07

- Added `docs/rotation-trigger-fgs-policy.md` and
  `docs/rotation-trigger-fgs-policy.json` as the checked policy packet for
  `RotationTriggerService` special-use foreground-service behavior.
- Added `tools/rotation_fgs_policy_check.py` plus focused backend tests so the
  manifest permission, service type, subtype property, source safeguards,
  Settings opt-in terms, Play declaration packet, owner evidence action, and
  workflow wiring cannot drift silently.
- Extended the Play App content packet with foreground-service declaration text,
  a `foregroundServices` JSON row, the
  `capture-foreground-service-declaration-evidence` owner action, and the
  corrected 14-permission current coverage count.
...
- Cycle 152 verification: rotation foreground-service policy check, Play App
  content packet check, release metadata consistency check, workflow security
  policy check, Python compile, focused tests, and full backend tool tests
  passed locally.

## Cycle 153 Result - 2026-06-07

- Added `docs/background-work-scheduling-ledger.md` and
  `docs/background-work-scheduling-ledger.json` as the checked WorkManager
  scheduling packet for periodic auto wallpaper, daily wallpaper, weather
  refresh, Aura Originals download, and rotation trigger one-shots.
- Added `tools/background_work_scheduling_check.py` plus focused backend tests
  so unique work names, worker classes, source paths, enqueue APIs, work
  policies, constraints, deferral reasons, source terms, release docs, and
  workflow wiring cannot drift silently.
- Wired verify and release workflows, release metadata consistency, GitHub
  security workflow policy, release dry-run, release signing, and supply-chain
  docs to run the new gate before Android build work.
- Cycle 153 verification: background work scheduling ledger check, release
...
## Cycle 158 Result - 2026-06-07

- Wired `CrashDiagnosticsCollector` to read `BackgroundWorkDiagnosticsReader`
  during bundle generation and append a `Background work live receipts` section
  when local reads succeed.
- Added live bundle formatting for metered/Data Saver state, WorkInfo state
  summary, WorkInfo record count, max attempts, last result, success/failure
  timestamps, last error class, and deferral reason.
- Preserved the static background-work support section as a fallback when live
  receipts cannot be read.
- Updated `docs/support/crash-diagnostics.md` and
  `docs/research/cycle-158-2026-06-07.md` for the support-bundle live receipt
  merge.
- Cycle 158 verification: focused `CrashDiagnosticsTextTest` and
  `SettingsViewModelTest` passed locally with Android Studio JBR.

## Cycle 159 Result - 2026-06-07

- Added `actionHint` to `BackgroundWorkStatusRow` and compute it from
  WorkInfo read errors, Data Saver restricted-background status, metered state,
  unmetered-only work, worker retry/failure receipts, source availability,
  provider/network I/O, Aura Originals validation, permission cues, apply
  failures, and WorkManager retry/failure states.
- Surfaced action hints in `Settings` > `Diagnostics` > `Background work` and
  copied/shared support bundles as `action=...` beside the raw deferral receipt.
- Tightened worker receipt copy for auto wallpaper, daily wallpaper, weather
  refresh, and Aura Originals download so repeated failures identify the likely
  recovery surface.
- Updated background-work scheduling/network policy docs,
  `docs/support/crash-diagnostics.md`, and
  `docs/research/cycle-159-2026-06-07.md` for the action-hint slice.
- Cycle 159 verification: focused `BackgroundWorkDiagnosticsReaderTest` and
  `CrashDiagnosticsTextTest` passed locally with Android Studio JBR.

## Next Cycle

Continue this same assigned project, Aura. Start Cycle 160 from the
`ROADMAP.md` Continuation State and
`docs/research/cycle-159-2026-06-07.md`. The account
...
added a checked rotation trigger boot behavior gate; Cycle 152 added a checked
rotation trigger foreground-service policy gate for the special-use service
permission, manifest subtype, source safeguards, Play Console declaration
packet, owner evidence action, and workflow wiring.
Cycle 153 added a checked background work scheduling ledger for `auto_wallpaper`,
...
Cycle 158 merged live WorkManager, Data Saver, and persisted background-work
receipt details into copied/shared crash diagnostics bundles.
Cycle 159 added Settings and support-bundle action hints for Data Saver
restrictions, metered waits, source failures, network/provider errors, Aura
Originals validation retries, permission cues, apply failures, and WorkManager
...
asset gap, background-work device/emulator scheduler evidence, release artifact
hardening, or the next checkable backend, deploy, security, support, policy, or
rules hardening artifact if owner-gated evidence is still
unavailable.
