...
- Check that every manifest permission has a reviewed Data safety matrix row before the signed release build.
...
`AndroidManifest.xml` gains, removes, or changes a permission without a reviewed
purpose, data type, collection/sharing, retention, deletion, denial, and Play
declaration row.
