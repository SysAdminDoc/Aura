...
vote, follow, block, sound upload, and wallpaper upload submission plus the
fail-closed pending profile callable, local Emulator Suite tests, Functions unit
...
- Functions report/vote/follow/block/sound-upload/wallpaper-upload handler
  status from `npm run test:functions` and the remaining fail-closed callable
  export list until emulator coverage replaces the pending handler status.
